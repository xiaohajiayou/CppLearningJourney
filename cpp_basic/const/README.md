...
+ 类型检查
  - const 是类型安全的
  - 它定义的常量具有特定的数据类型，编译器可以在编译时进行类型检查。例如，const int max_value = 100; 定义了一个整型常量 max_value，其值为100。
  - 需要和define区分，define只是文本替换，在汇编中会直接变为立即数，故没有类型


+ 防止修改，起保护作用，增加程序健壮性
    - 在汇编中，const定义的变量被存储在一个特定的内存位置，程序中通过内存地址来访问。 
...
## const内存分配
+ 可以节省空间，避免不必要的内存分配
    + const定义常量从汇编的角度来看，只是给出了对应的内存地址，const定义的常量在程序运行过程中通过地址访问，在内存中只有一份拷贝
    + 需要区别`#define`在汇编中被变成立即数，如果有多处使用，则这个常量在内存中存在若干个拷贝。
